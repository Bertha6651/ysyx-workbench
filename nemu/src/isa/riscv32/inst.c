...
void jump_all(word_t imm,word_t src1,int rd,Decode *s,int ifR,int ifCondition)
{
  if(ifSubOper(SEXT(imm,32))==1)
    if(ifR==0)
      s->dnpc =s->dnpc-(~SEXT(imm,32)+1)-4;
    else s->dnpc =((src1-(~SEXT(imm,12)+1))&(~1));
  else {
    if(ifR ==0) s->dnpc+=SEXT(imm,32)-4;
    else s->dnpc=((src1+SEXT(imm,12))&(~1));}
  if(ifCondition==0)
    R(rd)=s->pc+4;//有困难
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal     , J, jump_all(imm,0,rd,s,0,0));//有困难
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr    , I, jump_all(imm,src1,rd,s,1,0));
  //B
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq     , B, (src1==src2)?jump_all(imm,0,rd,s,0,1):0);//take the branch if registers rs1 and rs2 are equal .
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne     , B, (src1!=src2)?jump_all(imm,0,rd,s,0,1):0);//take the branch if registers rs1 and rs2 are unequal 
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt     , B, src1=noUOper(src1);src2=noUOper(src2);(src1<src2)?jump_all(imm,0,rd,s,0,1):0);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge     , B, src1=noUOper(src1);src2=noUOper(src2);(src1>=src2)?jump_all(imm,0,rd,s,0,1):0);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu    , B, (src1<src2)?jump_all(imm,0,rd,s,0,1):0);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu    , B, (src1>=src2)?jump_all(imm,0,rd,s,0,1):0);
  //L
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb      , I, R(rd) = SEXT(Mr(src1 + SEXT(imm,12), 1),XLEN));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh      , I, R(rd) = SEXT(Mr(src1 + SEXT(imm,12), 2),XLEN));//LH指令从存储器中读取一个16位数值
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu   , I, R(rd) = (src1<SEXT(imm,XLEN)?1:0));
